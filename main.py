...
def callback_inline(call):
    ...
    if call.data == 'mainmenu':
        ...
        status = -1
        choice = 0
        next_menu3 = types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton(text='Вернуться в начало', callback_data='mainmenu'))
        bot.edit_message_text('Чтобы записаться на тренинг пришлите свою почту', call.message.chat.id,
        ...
        bot.edit_message_text(
            'Хотите узнать больше о кибербезопасности?\nДля начала выясним текущий уровень ваших знаний об опасностях в интернете. Готовы?',
        ...
        if score in [1, 2]:  # Здесь будут индексы, чтобы определить какие темы проблемные
            ...
            next_menu4 = types.InlineKeyboardMarkup().add(
                types.InlineKeyboardButton(text='Вернуться в начало', callback_data='mainmenu'))
            bot.send_message(call.message.chat.id,
                             f'Тест завершен, вы набрали максимум баллов!\nУ вас хороший уровень знаний\nДля завершения подготовки остается пройти финальный тест!',
    ...
    if status == 1:  # прописать условие состояния
        if num_of_question != 4:
            if message.text == a_and_q[num_of_question][a_and_q[num_of_question][5]]:
                score += 1
                stage1 = types.InlineKeyboardMarkup().add(
                    types.InlineKeyboardButton(text='Следующий вопрос!', callback_data='key3'))
            ...
            if message.text == a_and_q[num_of_question][a_and_q[num_of_question][5]]:
                score += 1
                stage1 = types.InlineKeyboardMarkup().add(
                    types.InlineKeyboardButton(text='Завершить тест', callback_data='key4'))
                bot.send_message(message.from_user.id, 'Ответ правильный!', reply_markup=stage1)
            else:
                stage1 = types.InlineKeyboardMarkup().add(
                    types.InlineKeyboardButton(text='Завершить тест', callback_data='key4'))
